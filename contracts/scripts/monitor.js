...
class ContractMonitor {
  ...
  async monitorTransfers(fromBlock, toBlock) {
    ...
    for (const event of transferEvents) {
      ...
      if (value.gte(this.alertThresholds.largeTransfer)) {
        this.addAlert("LARGE_TRANSFER", `Large transfer detected: ${transfer.valueFormatted} USDT`, transfer);
      }
      
      // Check for zero address (mint/burn)
      if (from === ethers.constants.AddressZero) {
        this.addAlert("MINT_DETECTED", `Mint detected: ${transfer.valueFormatted} USDT to ${to}`, transfer);
      }
      
      if (to === ethers.constants.AddressZero) {
        this.addAlert("BURN_DETECTED", `Burn detected: ${transfer.valueFormatted} USDT from ${from}`, transfer);
      }
    }
    ...
    if (transferEvents.length > this.alertThresholds.rapidTransfers) {
      this.addAlert("RAPID_TRANSFERS", `High transfer activity: ${transferEvents.length} transfers`, {
  ...
  async monitorGovernance(fromBlock, toBlock) {
    ...
    console.log(`  📊 Processed ${proposalEvents.length} proposals, ${voteEvents.length} votes, ${executionEvents.length} executions`);
    
    // Check for high governance activity
    if (voteEvents.length > this.alertThresholds.governanceVotes) {
      this.addAlert("HIGH_GOVERNANCE_ACTIVITY", `High governance activity: ${voteEvents.length} votes`, {
  ...
  async monitorCompliance(fromBlock, toBlock) {
    ...
    console.log(`  📊 Processed ${blacklistEvents.length} blacklist, ${kycEvents.length} KYC, ${sanctionEvents.length} sanction updates`);
    ...
    if (newBlacklists > this.alertThresholds.blacklistAdditions) {
      this.addAlert("HIGH_BLACKLIST_ACTIVITY", `High blacklist activity: ${newBlacklists} additions`, {
        count: newBlacklists,
  ...
  async monitorSecurity(fromBlock, toBlock) {
    ...
    const [pausedEvents, unpausedEvents, roleGrantedEvents, roleRevokedEvents, upgradeEvents] = events;
    ...
    for (const event of unpausedEvents) {
      ...
      this.addAlert("CONTRACT_UNPAUSED", `Contract unpaused by ${unpauseInfo.account}`, unpauseInfo);
    }
    ...
    console.log(`  📊 Processed security events: ${pausedEvents.length + unpausedEvents.length + roleGrantedEvents.length + roleRevokedEvents.length + upgradeEvents.length}`);
  }
  ...
  async checkSystemHealth() {
    ...
    try {
      ...
      if (isPaused) {
        ...
        this.addAlert("HIGH_HOLDER_COUNT", `Holder count is ${healthStatus.token.utilizationRate} of maximum`, healthStatus);
      }
      
      if (governorCount.lt(3)) {
        this.addAlert("LOW_GOVERNOR_COUNT", `Only ${governorCount} governors configured`, healthStatus);
      }
      
      return healthStatus;
    } catch (error) {
      this.addAlert("HEALTH_CHECK_ERROR", `Health check failed: ${error.message}`, { error: error.message });
      return null;
    }
  }
...
async function main() {
  ...
  if (args.length < 2) {
    console.error("Usage: node scripts/monitor.js <token-address> <governance-address> [block-range] [--realtime]");
    process.exit(1);
  }
