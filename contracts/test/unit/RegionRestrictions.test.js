...
describe("Region Restrictions Comprehensive Tests", function () {
  let token;
  ...
  describe("Region Restrictions基础功能测试", function () {
    it("默认情况下，地区限制应该是禁用的", async function () {
      expect(await token.isRegionRestrictionsEnabled()).to.be.false;
    });

    it("应该能够启用地区限制功能", async function () {
  ...
  describe("Region Code管理测试", function () {
    ...
    it("不能为零地址设置地区代码", async function () {
      await expect(
    ...
    it("只有 COMPLIANCE_ROLE 可以设置地区代码", async function () {
      await expect(token.connect(user1).setRegionCode(user2.address, 1)).to.be.reverted;
    });

    it("应该能够设置多个不同的地区代码", async function () {
      await token.connect(complianceOfficer).setRegionCode(user1.address, 1); // 美国
  ...
  describe("Allowed Regions管理测试", function () {
    it("应该能够设置允许的地区", async function () {
  ...
  describe("Region Restriction逻辑测试", function () {
    ...
    it("当双方都在允许地区时，转账应该成功", async function () {
      // 设置地区代码
      await token.connect(complianceOfficer).setRegionCode(user1.address, 1); // 美国
    ...
    it("当发送方在禁止地区时，转账应该被阻止", async function () {
      // 设置地区代码
    ...
    it("当接收方在禁止地区时，转账应该被阻止", async function () {
      // 设置地区代码
      await token.connect(complianceOfficer).setRegionCode(user1.address, 1); // 美国
    ...
    it("当双方都在禁止地区时，转账应该被阻止", async function () {
      // 设置地区代码
      await token.connect(complianceOfficer).setRegionCode(user1.address, 999); // 禁止地区
    ...
    it("默认地区代码0应该被特殊处理", async function () {
      // 用户没有设置地区代码（默认为0）
      // 没有设置地区0为允许地区
    ...
    it("当允许地区0时，默认用户应该可以转账", async function () {
      // 允许地区0（默认地区）
      await token.connect(complianceOfficer).setAllowedRegion(0, true);
    ...
    it("跨允许地区的转账应该成功", async function () {
      // 设置不同的允许地区
      await token.connect(complianceOfficer).setRegionCode(user1.address, 1); // 美国
      await token.connect(complianceOfficer).setRegionCode(user2.address, 44); // 英国
  ...
  describe("Region Restrictions与其他限制的交互测试", function () {
    const transferAmount = ethers.parseUnits("100", 6);
    ...
    it("REGION_RESTRICTION 优先级应该低于黑名单检查", async function () {
      // 将发送方加入黑名单
    ...
    it("REGION_RESTRICTION 优先级应该低于制裁检查", async function () {
      // 将发送方加入制裁名单
    ...
    it("REGION_RESTRICTION 优先级应该低于KYC检查", async function () {
      // 移除发送方的KYC验证
    ...
    it("禁用地区限制后，地区限制应该不再生效", async function () {
      // 首先确认地区限制生效
  ...
  describe("Mint和Burn操作的地区限制测试", function () {
    const transferAmount = ethers.parseUnits("100", 6);
  ...
  describe("边界情况和错误处理测试", function () {
    it("应该能够处理大量地区代码", async function () {
    ...
    it("未设置地区代码的地址应该使用默认值0", async function () {
      await token.connect(complianceOfficer).setRegionRestrictionsEnabled(true);
      // 用户没有设置地区代码，默认为0
      // 没有允许地区0
    ...
    it("相同地区内的转账应该成功", async function () {
    ...
    it("自转账应该遵循地区限制规则", async function () {
      await token.connect(complianceOfficer).setRegionCode(user1.address, 999); // 禁止地区
      await token.connect(complianceOfficer).setAllowedRegion(1, true); // 只允许地区1
      await token.connect(complianceOfficer).setRegionRestrictionsEnabled(true);
  ...
  describe("批量地区管理测试", function () {
    it("应该能够批量设置多个地区代码", async function () {
    ...
    it("应该能够批量移除地区权限", async function () {
  ...
  describe("Message测试", function () {
    it("应该返回正确的限制消息", async function () {
