...
class ContractUpgrader {
  ...
  async upgradeViaGovernance(ContractFactory) {
    ...
    const upgradeData = this.contractType === 'token' 
      ? (await ethers.getContractAt("USDTToken", this.proxyAddress)).interface.encodeFunctionData("upgradeTo", [newImplementation.address])
      : (await ethers.getContractAt("USDTGovernance", this.proxyAddress)).interface.encodeFunctionData("upgradeTo", [newImplementation.address]);
...
async function prepareUpgradeProposal(contractType, proxyAddress, newImplementationAddress, governanceAddress) {
...
async function main() {
  ...
  if (args.length < 2) {
    console.error("Usage: node scripts/upgrade.js <contract-type> <proxy-address> [governance-address]");
