...
describe("INVALID_RECIPIENT 限制测试", () => {
  let token;
  ...
  describe("INVALID_RECIPIENT 限制逻辑测试", () => {
    ...
    it("向无效接收方转账应该返回 INVALID_RECIPIENT", async () => {
      const restrictionCode = await token.detectTransferRestriction(
        user1.address,
        invalidRecipient.address,
        transferAmount,
  ...
  describe("与其他限制的交互测试", () => {
    ...
    it("INVALID_RECIPIENT 优先级应该低于黑名单检查", async () => {
      // 将有效接收方加入黑名单
    ...
    it("INVALID_RECIPIENT 优先级应该低于制裁检查", async () => {
      // 将有效接收方加入制裁名单
  ...
  describe("边界情况测试", () => {
    ...
    it("当接收方验证未启用时，设置有效接收方应该无效果", async () => {
      // 接收方验证未启用
      expect(await token.isRecipientValidationRequired()).to.be.false;
