...
class SecurityChecker {
  ...
  async checkAccessControls() {
    ...
    try {
      ...
      const governanceHasUpgraderRole = await this.token.hasRole(
        UPGRADER_ROLE,
      ...
      this.addCheck({
        ...
        status:
          governanceHasMinterRole && governanceHasPauserRole && governanceHasUpgraderRole
            ? "PASS"
            : "WARN",
        details: {
        ...
        recommendations: !governanceHasMinterRole
          ? ["Consider granting MINTER_ROLE to governance"]
          : [],
      });
  ...
  async checkEmergencyFunctions() {
    ...
    try {
      ...
      this.addCheck({
        ...
        recommendations: pauserCount.eq(0)
          ? ["No pausers assigned - emergency response may be compromised"]
          : [],
      });
  ...
  async checkComplianceSettings() {
    ...
    try {
      ...
      console.log(
        `  ✅ Transfer Limits: ${ethers.utils.formatUnits(minTransferAmount, 6)} - ${ethers.utils.formatUnits(maxTransferAmount, 6)}`,
      );
    } catch (error) {
      this.addCheck({
        category: "Compliance Settings",
  ...
  async checkGovernanceConfiguration() {
    ...
    try {
      ...
      this.addCheck({
        ...
        recommendations: !governanceSecure
          ? ["Consider increasing governor count and required votes for better security"]
          : [],
      });
  ...
  async checkUpgradeability() {
    ...
    try {
      ...
      const governanceImplementation = await upgrades.erc1967.getImplementationAddress(
        this.governanceAddress,
      );
  ...
  async checkEventEmissions() {
    ...
    try {
      ...
      const blacklistEvents = await this.token.queryFilter(
        "BlacklistUpdated",
        fromBlock,
        currentBlock,
      );
      const proposalEvents = await this.governance.queryFilter(
        "ProposalCreated",
        fromBlock,
        currentBlock,
      );
